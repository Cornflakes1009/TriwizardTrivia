//
//  ClassicPlayViewController.swift
...
class ClassicPlayViewController: UIViewController, GADInterstitialDelegate {
    ...
    let optionZeroButton: GameButton = {
        ...
        button.addTarget(nil, action: #selector(answerTapped), for: .touchUpInside)
    ...
    func setupViews() {
        ...
        let questionLabelHeight = CGFloat(screenHeight / 4)
        view.addSubview(questionLabel)
        questionLabel.anchor(top: view.safeAreaLayoutGuide.topAnchor, left: view.leftAnchor, bottom: nil, right: view.rightAnchor, paddingTop: 30, paddingLeft: 20, paddingBottom: 0, paddingRight: 20, width: 0, height: questionLabelHeight)
    ...
    func checkIfCorrect(buttonNumber: Int) {
        ...
        if buttonNumber == soloQuestionList[soloQuestionIndex].answer {
            ...
            if soloQuestionIndex + 1 != soloQuestionList.count {
                ...
                showInterstitial()
                let vc = ClassicScoreResultsViewController()
